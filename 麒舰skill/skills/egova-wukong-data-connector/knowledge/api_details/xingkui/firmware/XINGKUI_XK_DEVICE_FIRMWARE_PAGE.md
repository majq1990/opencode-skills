...
| 项目 | 内容 |
| --- | --- |
| apiCode | XINGKUI_XK_DEVICE_FIRMWARE_PAGE |
| domain | 星揆 |
| bizObject | 设备固件 |
| apiName | 分页查询 |
| apiType | page |
...
该接口用于分页查询设备固件数据，支持按id、固件id、固件包的文件名，包括文件后缀、固件版本（需要根据官方固件版本进行格式化：00.00.0000）、存储桶中固件包的对象密钥、固件包的大小、固件包的md5码、工作区id等条件筛选；返回分页结果，适合分页表格或列表组件。
...
- `result` 类型：`PageResult<XkDeviceFirmware>`
- 标准化响应形态：`array`
