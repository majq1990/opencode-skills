...
| 项目 | 内容 |
| --- | --- |
| apiCode | XINGKUI_XK_DEVICE_FIRMWARE_LIST |
| domain | 星揆 |
| bizObject | 设备固件 |
| apiName | 列表查询 |
| apiType | list |
...
该接口用于查询设备固件列表，支持按id、固件id、固件包的文件名，包括文件后缀、固件版本（需要根据官方固件版本进行格式化：00.00.0000）、存储桶中固件包的对象密钥、固件包的大小、固件包的md5码、工作区id等条件筛选；返回列表数据，适合表格、下拉或列表组件。
...
- `result` 类型：`List<XkDeviceFirmware>`
- 标准化响应形态：`array`
