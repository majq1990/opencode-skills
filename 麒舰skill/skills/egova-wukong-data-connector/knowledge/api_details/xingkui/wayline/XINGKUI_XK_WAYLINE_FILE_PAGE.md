...
| 项目 | 内容 |
| --- | --- |
| apiCode | XINGKUI_XK_WAYLINE_FILE_PAGE |
| domain | 星揆 |
| bizObject | 航线文件 |
| apiName | 分页查询 |
| apiType | page |
...
该接口用于分页查询航线文件数据，支持按id、航线名称、航线id、设备产品枚举（格式：domain-device_type-sub_type）、负载产品枚举（格式：domain-device_type-sub_type）、当前航线所属的工作区、航线文件的md5码、是否最受欢迎等条件筛选；返回分页结果，适合分页表格或列表组件。
...
- `result` 类型：`PageResult<XkWaylineFile>`
- 标准化响应形态：`array`
