...
| 项目 | 内容 |
| --- | --- |
| apiCode | XINGKUI_XK_DEVICE_PAYLOAD_LIST |
| domain | 星揆 |
| bizObject | 设备负载 |
| apiName | 列表查询 |
| apiType | list |
...
该接口用于查询设备负载列表，支持按id、负载设备序列号、负载设备型号（对应设备字典里的设备型号）、负载设备类型（对应设备字典里的设备类型）、负载设备子类型（对应设备字典里的设备子类型）、负载设备的固件版本、负载设备的位置、负载设备所属设备的序列号等条件筛选；返回列表数据，适合表格、下拉或列表组件。
...
- `result` 类型：`List<XkDevicePayload>`
- 标准化响应形态：`array`
