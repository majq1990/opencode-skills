...
| 项目 | 内容 |
| --- | --- |
| apiCode | XINGKUI_XK_RC_DEVICE_PAGE |
| domain | 星揆 |
| bizObject | 遥控器设备 |
| apiName | 分页查询 |
| apiType | page |
...
该接口用于分页查询遥控器设备数据，支持按id、设备序列号等条件筛选；返回分页结果，适合分页表格或列表组件。
...
| 数据库 | 数据表 | 备注 |
| --- | --- | --- |
| xingkui | gis_xk_rc_device | 从返回实体 `@Table` 推断 |
...
- `result` 类型：`PageResult<XkRcDevice>`
- 标准化响应形态：`array`
