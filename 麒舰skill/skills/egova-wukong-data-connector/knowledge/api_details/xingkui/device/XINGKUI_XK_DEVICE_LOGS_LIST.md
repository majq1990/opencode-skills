...
| 项目 | 内容 |
| --- | --- |
| apiCode | XINGKUI_XK_DEVICE_LOGS_LIST |
| domain | 星揆 |
| bizObject | 设备日志 |
| apiName | 列表查询 |
| apiType | list |
...
该接口用于查询设备日志列表，支持按id、日志id、创建者、设备序列号、日志问题的描述、日志记录问题发生的时间、日志问题状态（1：上传；2：完成；3：取消；4:失败）、更新时间等条件筛选；返回列表数据，适合表格、下拉或列表组件。
