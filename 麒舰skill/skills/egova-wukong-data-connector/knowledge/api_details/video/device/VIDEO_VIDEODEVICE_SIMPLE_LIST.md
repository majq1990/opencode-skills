...
| 项目 | 内容 |
| --- | --- |
| apiCode | VIDEO_VIDEODEVICE_SIMPLE_LIST |
| domain | 视频 |
| bizObject | 视频设备 |
| apiName | 视频设备简要列表 |
| apiType | list |
...
该接口用于查询视频设备简要列表，默认按配置字段返回，也支持通过 multiFields 追加自定义返回字段；支持按设备名称、类型、状态、目录、区域、关键字、关注等条件筛选，适合表格、列表或地图图层。
...
| multiFields | List<Map<String,Object>> | 否 |  |  | 自定义返回字段配置 |
| sorts | Sorting[] | 否 |  |  | 排序规则 |
...
- `result` 类型：`List<Map<String, Object>`
- 标准化响应形态：`array`
