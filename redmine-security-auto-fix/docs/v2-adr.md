...
## 7. 2026-09-24 追加决策：外部情报走服务器关注面定向，不推钉钉台账

原设计里 CVE 情报有两个出口：一是 `collect_cve_intel.py` 跨目录引用 vuln-response 的
抓取脚本，二是把情报映射到 vuln-response 的钉钉「安全漏洞台账」AI 表格。两条都改了。

**决策 A：情报采集从 vuln-response 改为服务器 sec_kb 的关注面定向抓取。**
理由是 vuln-response 的口径（NVD 近期全量 + GHSA 按严重级别）没有靶子：实测 504 条
情报里公司环境关注的 tomcat / nginx / redis / mysql / nacos / kafka / log4j / spring /
openssl 命中数全为 0，Top1 是 mcp-atlassian。sec_kb 从 9,361 条安全案件与安全文档语料
反推公司真实暴露面（121 项），拿它当 NVD `keywordSearch` 的关键词、并作为查询时的情报
闸门。改造后 843 条情报放行 331 条（39.3%），命中组件全是公司真实在跑的组件。
`collect_cve_intel.py` 保留为离线兜底，服务器不可达时才走。

**决策 B：不把 `sec_intel` 推送到钉钉「安全漏洞台账」。**
该表每行要求 `[修复] ... | [验证] ... | [公告] ...` 三段式，其中修复命令必须来自厂商
逐操作系统公告；NVD/GHSA 情报不带这类信息，凭情报编造修复命令既不可信也触犯第 6 节
红线。同时 504 条情报命中公司 OS 的为 0——写进去只会把第三方库 CVE 灌进一张操作系统
台账。情报的正确位置是 `query` 的 `external_intel` 段，只补充漏洞事实（CVSS / 受影响
版本 / 厂商公告链接），在建议优先级里排第 6 位且不算修复建议。

**代价**：关注面是语料派生的，公司上了新组件而语料还没提到时会漏（SEED 种子表兜底，
但不是全量）；cron 每日 03:50 重建关注面、04:30 定向抓取，存在一天的滞后窗口。
