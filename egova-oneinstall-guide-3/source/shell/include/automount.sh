...
FILE="/etc/yum.repos.d/egova-oneinstall-local.repo"
...
install_lvm2_rpm() {
    ...
    if test $? -ne 0; then
        # 在脚本开始前执行的，不展示安装信息
        if is_ubuntu; then
          dpkg  -i src/repo/libdevmapper-event*.deb
          dpkg  -i src/repo/libreadline5*.deb
          dpkg  -i src/repo/lvm2_*deb*
        elif  is_centos; then
          rpm -ivh src/repo/${release_ver}/device-mapper-*.rpm --nodeps --force
          rpm -ivh src/repo/${release_ver}/libaio-*.rpm
          rpm -ivh src/repo/${release_ver}/lvm2-*.rpm --nodeps --force
        else
          rpm -ivh src/repo/${release_name}/${release_ver}/device-mapper-event-*.rpm --nodeps --force
          rpm -ivh src/repo/${release_name}/${release_ver}/libaio-*.rpm --nodeps --force
          rpm -ivh src/repo/${release_name}/${release_ver}/thin-provisioning-tools-*.rpm --nodeps --force
          rpm -ivh src/repo/${release_name}/${release_ver}/lvm2-*.rpm --nodeps --force
        fi
    fi
