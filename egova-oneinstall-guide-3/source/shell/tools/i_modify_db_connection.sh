...
db_template_file=../template/db_template.yml
...
function display_mc_select() {
  ...
  # 根据用户选择的索引找到对应的 key
    ...
    if { [[ "${SELECTED_PRODUCT_NAME}" == "export" ]] || [ "${SELECTED_PRODUCT_NAME}" == "evaluation" ]; }; then
        input_database_service_connection_info ${DB_TYPE}
    else
       input_database_connection_info ${DB_TYPE}
    fi
}
function display_service_select() {
  local key=$1
  local count=$(yq ".${key}.sub_options | length " ${db_template_file})
  local index=0
  echo "请选择产品类型："
  while [ $index -lt $count ]; do
    local server_name=$(yq ".${key}.sub_options[${index}].name" ${db_template_file})
    ((index = index + 1))
    echo "${index}: ${server_name}"
  done
  echo "q: 退出"
...
function display_database_type_select() {
  local keys=($(yq '.[] | select(.display=="1") | key ' ${db_template_file} ))
  echo "请选择一键切换数据库类型："
  for key in "${keys[@]}"; do
    local name=$(yq ".${key}.name" ${db_template_file})
    local choice_index=$(yq ".${key}.choice_index" ${db_template_file})
  ...
  case "${Select}" in
    1)
      ...
      display_service_select ${DB_TYPE}
      ;;
    2)
      DB_TYPE="postgresql"
...
function input_database_connection_info() {
    ...
    read -ep '请输入数据库用户密码:' DB_PASSWORD
    read -ep '请输入数据库Schema:' DB_SCHEMA
}

function input_database_service_connection_info() {
    local db_type=$1
    echo "Database type ${db_type}"
    Echo_Yellow "请输入业务数据库连接信息"
    read -ep '请输入业务数据库IP:' DB_HOST
    if [ "${db_type}" != "dm" ]; then
      read -ep '请输入业务数据库实例名称:' DB_NAME
    fi
    read -ep '请输入业务数据库端口:' DB_PORT
    read -ep '请输入业务数据库用户名:' DB_USER
    read -ep '请输入业务数据库用户密码:' DB_PASSWORD
    read -ep '请输入业务数据库Schema:' DB_SCHEMA

    Echo_Yellow "请输入统计数据库连接信息"
    read -ep '请输入统计数据库IP:' DB_STAT_HOST
    if [ "${db_type}" != "dm" ]; then
      read -ep '请输入统计数据库实例名称:' DB_STAT_NAME
    fi
    read -ep '请输入统计数据库端口:' DB_STAT_PORT
    read -ep '请输入统计数据库Schema:' DB_STAT_SCHEMA
}
...
#     fi
    Echo_Yellow "开始一键切换${DB_TYPE}数据库"
    local nodes=($(yq '.all.hosts[] | key' $hosts_file))
    for node in "${nodes[@]}"; do
       ansible-playbook -i $hosts_file -e "host=${node} SELECTED_PRODUCT_NAME=${SELECTED_PRODUCT_NAME} DB_STAT_HOST=${DB_STAT_HOST} DB_STAT_NAME=${DB_STAT_NAME} DB_STAT_PORT=${DB_STAT_PORT} DB_STAT_SCHEMA=${DB_STAT_SCHEMA}  DB_HOST=${DB_HOST} DB_PORT=${DB_PORT} DB_USER=${DB_USER} DB_PASSWORD=${DB_PASSWORD} DB_NAME=${DB_NAME} DB_SCHEMA=${DB_SCHEMA} DB_TYPE=${DB_TYPE} DB_DRIVER=${DB_DRIVER} " ../../ansible/modify_db.yml
    done
