...
function download_fileslist(){
    ...
    if [ "${base_select}" == "1" ];then
        downfile_with_auth ${domain}/one/v2/code_test/oneinstall_v2-files-base.ini
    elif [ "${base_select}" == "2" ];then
	    downfile_with_auth ${domain}/one/v2/code_test/oneinstall_v2-files-base.ini
        downfile_with_auth ${domain}/one/v2/code_test/iniconfig/oneinstall_v2-files-env-base.ini
    fi
    if [ "$script_select" != "0" ] ; then
        downfile_with_auth ${domain}/one/v2/code_test/iniconfig/${INSTALL_FILE_ARRAY[$(($script_select - 1))]}
    ...
    while IFS= read -r file; do
        ...
        if [[ ! $file =~ ^\[ ]] && [[ -n "$file" ]]; then
            if [ "$type" == "all" ] || [[ "$file" == *"$type"* ]]; then
                downfile_with_auth "${domain}/one/v2/${file}"
            fi
        fi
    done < "$ini"
}
