...
# 建表
mysql ${connect_db_cmd} -D ${database} -e "CREATE TABLE IF NOT EXISTS db_init_log_oneinstall(id bigint NOT NULL AUTO_INCREMENT, sql_file varchar(255) NOT NULL, execute_time timestamp NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;"  2>/dev/null
...
do
    ...
    # 注释掉 db_init_log_oneinstall 表的插入语句
    sed -i "s/^INSERT INTO \`db_init_log_oneinstall\`/-- INSERT INTO \`db_init_log_oneinstall\`/g" ${sql_file}
    sed -i "s/^INSERT INTO \`${database}\`.\`db_init_log_oneinstall\`/-- INSERT INTO \`${database}\`.\`db_init_log_oneinstall\`/g" ${sql_file}
    echo "执行sql: ${sql_file}"
