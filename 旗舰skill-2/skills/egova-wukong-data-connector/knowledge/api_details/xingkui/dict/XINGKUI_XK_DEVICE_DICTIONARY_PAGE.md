...
| 项目 | 内容 |
| --- | --- |
| apiCode | XINGKUI_XK_DEVICE_DICTIONARY_PAGE |
| domain | 星揆 |
| bizObject | 设备字典 |
| apiName | 分页查询 |
| apiType | page |
...
该接口用于分页查询设备字典数据，支持按id、设备域（对应于设备字典表 0：飞行器；1：负载；2：遥控器；3：机场）、类型、子类型、设备型号、设备描述等条件筛选；返回分页结果，适合分页表格或列表组件。
...
- `result` 类型：`PageResult<XkDeviceDictionary>`
- 标准化响应形态：`array`
