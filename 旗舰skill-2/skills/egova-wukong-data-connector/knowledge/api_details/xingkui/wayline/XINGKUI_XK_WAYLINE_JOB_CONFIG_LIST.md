...
| 项目 | 内容 |
| --- | --- |
| apiCode | XINGKUI_XK_WAYLINE_JOB_CONFIG_LIST |
| domain | 星揆 |
| bizObject | 航线任务配置 |
| apiName | 列表查询 |
| apiType | list |
...
该接口用于查询航线任务配置列表，支持按id、配置id、任务名称、任务使用的航线文件id、执行任务的设备序列号、任务执行的主体0:dock,1:rc、任务所属的工作区id、任务类型等条件筛选；返回列表数据，适合表格、下拉或列表组件。
...
- `result` 类型：`List<XkWaylineJobConfig>`
- 标准化响应形态：`array`
