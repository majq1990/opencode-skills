...
| 项目 | 内容 |
| --- | --- |
| apiCode | XINGKUI_XK_WAYLINE_FILE_COUNT |
| domain | 星揆 |
| bizObject | 航线文件 |
| apiName | 数量统计 |
| apiType | count |
...
该接口用于统计航线文件数量，支持按id、航线名称、航线id、设备产品枚举（格式：domain-device_type-sub_type）、负载产品枚举（格式：domain-device_type-sub_type）、当前航线所属的工作区、航线文件的md5码、是否最受欢迎等条件筛选；返回值为数量数值，不返回明细列表，适合总数卡片或指标卡。
