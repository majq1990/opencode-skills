...
| 项目 | 内容 |
| --- | --- |
| apiCode | XINGKUI_XK_MEDIA_FILE_COUNT |
| domain | 星揆 |
| bizObject | 媒体文件 |
| apiName | 数量统计 |
| apiType | count |
...
该接口用于统计媒体文件数量，支持按id、文件id、文件名称、文件路径、文件所属工作区id、文件的指纹，此属性仅适用于Pilot上载的媒体文件。、文件的微小指纹，此属性仅适用于Pilot上载的媒体文件。、存储桶中的key等条件筛选；返回值为数量数值，不返回明细列表，适合总数卡片或指标卡。
