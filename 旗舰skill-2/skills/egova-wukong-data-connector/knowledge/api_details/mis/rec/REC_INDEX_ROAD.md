...
| 星桥接口路径地址 | `API平台/悟能接口/城管接口/案件相关/基于道路的案件指标统计` |
| 数据提交方式 | application/json |
| 响应主路径 | `result` |
| 响应形态 | array |
| 适配组件 | 柱状图、道路排行、TopN 列表、道路案件统计 |

---

## 2. 接口说明

该接口是“案件指标统计接口”的道路维度深化版。

用于统计经过条件筛选后，各个道路的案件指标值。可根据区划、案件来源、问题类型等条件过滤后，再按道路统计指标。
...
| 参数名 | 类型 | 是否必须 | 参数说明 |
|---|---|---|---|
| roadLevelIdList | Integer[] | 否 | 查询指定道路级别 |
| roadIdList | String[] | 否 | 查询指定道路 |
...
| result[].name | name | String | 是 | 道路 id | 可映射为 `id` |
| result[].text | text | String | 是 | 道路名称 | 可映射为 `name` / `label` |
