...
| 项目 | 内容 |
| --- | --- |
| apiCode | VIDEO_VIDEODEVICE_V1_TREE_CATALOG |
| domain | 视频 |
| bizObject | 视频分组 |
| apiName | 视频根据catalog展示树形V1 |
| apiType | tree |
| 请求方式 | POST |
...
该接口用于查询视频分组树形数据，支持按id等条件筛选；返回树节点列表，适合树组件或地图图层树。
...
- `result` 类型：`List<Tree<VideoDeviceCatalog>`
- 标准化响应形态：`array`
...
| +grade | String | 否 | 数据库字段：`grade` | 级别 |
| +parentId | String | 否 | 数据库字段：`parent_id` | 上级节点id |
