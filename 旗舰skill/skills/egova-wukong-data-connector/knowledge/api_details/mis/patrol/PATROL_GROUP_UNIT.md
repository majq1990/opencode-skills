...
| 星桥接口路径地址 | `API平台/悟能接口/城管接口/人员相关/人员部门分组` |
| 数据提交方式 | application/json |
| 响应主路径 | `result` |
| 响应形态 | array |
| 适配组件 | 柱状图、排行列表、部门人员统计、部门指标卡 |

---

## 2. 接口说明

该接口用于统计指定查询条件下，每个部门的人员总数。
...
> 注意：该接口是按部门分组，要展示的部门由 `unitCondition` 指定，因此使用该接口必须指定部门查询条件。
...
| result | result | Object[] | 是 | 部门分组结果 | 图表数据源 |
...
3. 统计下级部门归属时，开启 `unitExtendFlag`。
4. 如通过区划过滤人员，可配合 `regionId` 和 `regionExtendFlag`。
5. 如果需要树形结构，应使用人员部门分组树形接口。