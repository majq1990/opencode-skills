...
| 项目 | 内容 |
| --- | --- |
| apiCode | XINGKUI_XK_MEDIA_FILE_PAGE |
| domain | 星揆 |
| bizObject | 媒体文件 |
| apiName | 分页查询 |
| apiType | page |
...
该接口用于分页查询媒体文件数据，支持按id、文件id、文件名称、文件路径、文件所属工作区id、文件的指纹，此属性仅适用于Pilot上载的媒体文件。、文件的微小指纹，此属性仅适用于Pilot上载的媒体文件。、存储桶中的key等条件筛选；返回分页结果，适合分页表格或列表组件。
...
- `result` 类型：`PageResult<XkMediaFile>`
- 标准化响应形态：`array`
