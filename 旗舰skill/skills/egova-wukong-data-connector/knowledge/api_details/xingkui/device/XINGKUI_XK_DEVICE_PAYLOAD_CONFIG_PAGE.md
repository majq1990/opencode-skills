...
| 项目 | 内容 |
| --- | --- |
| apiCode | XINGKUI_XK_DEVICE_PAYLOAD_CONFIG_PAGE |
| domain | 星揆 |
| bizObject | 设备负载配置 |
| apiName | 分页查询 |
| apiType | page |
...
该接口用于分页查询设备负载配置数据，支持按id、设备负载编码、对应AI的设备编码、设备编码等条件筛选；返回分页结果，适合分页表格或列表组件。
...
| 数据库 | 数据表 | 备注 |
| --- | --- | --- |
| xingkui | gis_xk_device_payload_config | 从返回实体 `@Table` 推断 |
...
| --- | --- | --- | --- | --- | --- |
| id | String | 否 |  |  | id |
| payloadSn | String | 否 |  |  | 设备负载编码 |
| deviceCode | String | 否 |  |  | 对应AI的设备编码 |
| deviceSn | String | 否 |  |  | 设备编码 |
...
- `result` 类型：`PageResult<XkDevicePayloadConfig>`
- 标准化响应形态：`array`
...
| +payloadSn | String | 否 | 数据库字段：`payload_sn` | 设备负载编码 |
| +deviceCode | String | 否 | 数据库字段：`device_code` | 对应AI的设备编码 |
| +deviceSn | String | 否 | 数据库字段：`device_sn` | 设备编码 |
