...
| 项目 | 内容 |
| --- | --- |
| apiCode | XINGKUI_XK_DEVICE_LOGS_PAGE |
| domain | 星揆 |
| bizObject | 设备日志 |
| apiName | 分页查询 |
| apiType | page |
...
该接口用于分页查询设备日志数据，支持按id、日志id、创建者、设备序列号、日志问题的描述、日志记录问题发生的时间、日志问题状态（1：上传；2：完成；3：取消；4:失败）、更新时间等条件筛选；返回分页结果，适合分页表格或列表组件。
...
| 数据库 | 数据表 | 备注 |
| --- | --- | --- |
| xingkui | gis_xk_device_logs | 从返回实体 `@Table` 推断 |
...
| --- | --- | --- | --- | --- | --- |
| id | String | 否 |  |  | id |
| logsId | String | 否 |  |  | 日志id |
| username | String | 否 |  |  | 创建者 |
| deviceSn | String | 否 |  |  | 设备序列号 |
| logsInfo | String | 否 |  |  | 日志问题的描述 |
| happenTime | Long | 否 |  |  | 日志记录问题发生的时间 |
| status | Integer | 否 |  |  | 日志问题状态（1：上传；2：完成；3：取消；4:失败） |
...
- `result` 类型：`PageResult<XkDeviceLogs>`
- 标准化响应形态：`array`
...
| +logsId | String | 否 | 数据库字段：`logs_id` | 日志id |
| +username | String | 否 | 数据库字段：`username` | 创建者 |
| +deviceSn | String | 否 | 数据库字段：`device_sn` | 设备序列号 |
| +logsInfo | String | 否 | 数据库字段：`logs_info` | 日志问题的描述 |
| +happenTime | Long | 否 | 数据库字段：`happen_time` | 日志记录问题发生的时间 |
| +status | Integer | 否 | 数据库字段：`status` | 日志问题状态（1：上传；2：完成；3：取消；4:失败） |
