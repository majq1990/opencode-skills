...
| 项目 | 内容 |
| --- | --- |
| apiCode | XINGKUI_XK_DOCK_DEVICE_COUNT |
| domain | 星揆 |
| bizObject | 机场设备 |
| apiName | 数量统计 |
| apiType | count |
...
该接口用于统计机场设备数量，支持按id、设备序列号、机场状态代码(0:空闲中；1:现场调试；2:远程调试；3:固件升级中；4:作业中)、使用范围、应急范围、是否模拟飞行：0（否），1（是）等条件筛选；返回值为数量数值，不返回明细列表，适合总数卡片或指标卡。
