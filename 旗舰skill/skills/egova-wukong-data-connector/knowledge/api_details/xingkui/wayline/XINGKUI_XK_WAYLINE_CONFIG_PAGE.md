...
| 项目 | 内容 |
| --- | --- |
| apiCode | XINGKUI_XK_WAYLINE_CONFIG_PAGE |
| domain | 星揆 |
| bizObject | 航线配置 |
| apiName | 分页查询 |
| apiType | page |
...
该接口用于分页查询航线配置数据，支持按id、航线名称、航线类型、前端显示的航线类型。0：航点航线；1：块状航线 2：带状航线、机场系列号、巡检半径、参考起飞点经度、参考起飞点纬度等条件筛选；返回分页结果，适合分页表格或列表组件。
...
- `result` 类型：`PageResult<XkWaylineConfig>`
- 标准化响应形态：`array`
